...
impl Database {
    ...
    /// v16 -> v17 迁移：添加 TPS 监控样本表（本地定制）
    /// 与 create_tables_on_conn 中的建表语句保持一致（IF NOT EXISTS 保证幂等）。
    fn migrate_v16_to_v17(conn: &Connection) -> Result<(), AppError> {
        // 兜底：本地旧构建可能已把开发库标记为 v13+ 从而跳过 12=> 迁移臂，
        // 这里幂等补齐 input_token_semantics 列，避免新版 25 列 INSERT 运行时失败。
        if Self::table_exists(conn, "proxy_request_logs")? {
            Self::add_column_if_missing(
                conn,
                "proxy_request_logs",
        ...
        .map_err(|e| AppError::Database(format!("v16 -> v17 创建 tps_samples 表失败: {e}")))?;
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_tps_samples_created_at ON tps_samples(created_at DESC)",
            [],
        )
        .map_err(|e| {
            AppError::Database(format!("v16 -> v17 创建 tps_samples 时间索引失败: {e}"))
        })?;
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_tps_samples_app_created ON tps_samples(app_type, created_at DESC)",
            [],
